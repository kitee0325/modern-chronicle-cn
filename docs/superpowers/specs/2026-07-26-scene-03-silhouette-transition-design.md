...
本节覆盖第 7、8、9 节中与 Scene 03 镜头节奏冲突的旧约束。Scene 03
总时长调整为 `5.2` 个时间轴单位，仍由同一个 ScrollTrigger 主时间轴 scrub
控制，不使用定时器或第二个滚动触发器。
...
停驻中心使用“左上文本块、人物／词云、数据来源”三块区域的最小外接矩形，
而不是人物单点。左上文本块采用 Scene 03 后段文本块 `s03-copy-later`，
其 2× 画布左边界为 `x=2054×2=4108`；人物／词云与数据来源的透明像素
外接范围右边界为 `x=6973`。三块区域的横向外接矩形中心为：

```text
groupCenterX = (4108 + 6973) / 2 = 5540.5
```

运行时根据 Scene 03 slot 的实际宽度和当前 viewport 宽度换算为镜头进度：

```text
centerProgress =
  clamp((slotWidth × 5540.5 / 8588 - viewportWidth / 2)
        / (slotWidth - viewportWidth), 0, 1)
```

Scene 03 的镜头动画拆为两段线性 tween。第一段停在 `centerProgress`；原人像
定格、词云渐变和阅读停留期间，所有 track surface 的 x 坐标保持不变；第二段
从同一坐标继续移动至原有镜头终点。延迟出现的内容按实际两段镜头位置重新映射，
不得在定格区间内因旧的线性时间映射提前出现。

长卷轨道保持全高铺满，只做水平运动，因此外接矩形执行横向居中，不增加纵向
位移，避免画布上下边缘露出空白。
...
scene03:centered
scene03:morph-start
scene03:morph-complete
scene03:resume
```

`prefers-reduced-motion` 保留居中和两段停留，但取消词云缩放，在
`scene03:morph-start` 直接切换状态。静态预览必须使用同一分段函数计算镜头位置
和词云状态，确保截图、响应式检查与真实滚动路径一致。
